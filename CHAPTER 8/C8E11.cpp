...
struct maxminmedmean
{
    int smallest;
    int largest;
    double mean;
    double median;
    maxminmedmean(int a, int b, double c, double d): smallest(a), largest(b), mean(c), median(d) {}
};
...
double median(const vector<double>& v)
{
    ...
    if (h.size()%2==0)
    {
        return (h[(h.size()/2)-1]+h[h.size()/2])/2;
    }
    else return h[(h.size()-1)/2];
...
maxminmedmean maxv(const vector<double>& v)
{
    double high = v[0];
    int high_int=0;
    double low = v[0];
    ...
    return maxminmedmean(low_int,high_int,mean(v),median(v));   
...
int main()
{
    vector<double>price={1.1,0,2.2,3.3};
    vector<double>weight={1,2,3};
    maxminmedmean h = maxv(price);
    cout<<" smallest(index): "<<h.smallest
        <<" biggest(index): "<<h.largest
        <<" median: "<<h.median
        <<"mean: "<<h.mean;
}